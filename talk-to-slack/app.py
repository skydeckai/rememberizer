...
def auth_rememberizer():
    ...
    auth_url = (
        f"https://api.ffaxx.com/api/v1/auth/oauth2/authorize?client_id={REMEMBERIZER_CLIENT_ID}"
...
def auth_rememberizer_callback():
    ...
    token_url = "https://api.ffaxx.com/api/v1/auth/oauth2/token/"
    redirect_uri = request.url_root + "auth/rememberizer/callback"
...
def dashboard():
    ...
    response = requests.get("https://api.ffaxx.com/api/v1/account", headers=headers)
...
def slack_info():
    ...
    try:
        response = requests.get(
            "https://api.ffaxx.com/api/v1/integrations", headers=headers
        )
        if response.status_code != 200:
            return redirect("/auth/rememberizer")
        ...
        if slack_integration:
            documents_response = requests.get(
                f"https://api.ffaxx.com/api/v1/documents?integration_type=slack",
                headers=headers,
            )
